...
! operations  done a priori to speed up the code AAG & IS  07/03/06
          implicit none
          integer iter
          real one_minus_omegarelax, abse,one_over_nxnynz,res_red_factor
...
! loops change for 1, imax-1 to 2,imax-2, AAG & IS 07/03/06
          do iter=1,itermax
             ...
             do k=2,nz-2
                do j=2,ny-2
                   do i=2,nx-2
                      p1(i,j,k)=denom(i,j,k)*((e(i,j,k)*p1(i+1,j,k)+f(i,j,k)*p1(i-1,j,k)) &
...
! calculating residual, AAG 07/03/06
             abse=sum(abs(p1-p2))*one_over_nxnynz
