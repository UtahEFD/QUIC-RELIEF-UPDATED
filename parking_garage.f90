         ...
         do k=kstart(ibuild),kend(ibuild)
            do j=jstart(ibuild),jend(ibuild)
               do i=istart(ibuild),iend(ibuild)
                  if(icellflag(i,j,k) .ne. 0 .and. icellflag(i,j,k+1) .eq. 0)then
                     uo(i,j,k)  =uo(i,j,k)*K_garage
                     if(icellflag(i+1,j,k+1) .ne. 0)uo(i+1,j,k)=uo(i+1,j,k)*K_garage
                     vo(i,j,k)  =vo(i,j,k)*K_garage
                     if(icellflag(i,j+1,k+1) .ne. 0)vo(i,j+1,k)=vo(i,j+1,k)*K_garage
