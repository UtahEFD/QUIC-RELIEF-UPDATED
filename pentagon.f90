...
!now update flow features
            Weff(ibuild)=2.*radius_out
            Leff(ibuild)=2.*radius_out
                        ...
                        if(R_u .gt. radius_in .and.   &
                           ...
                           if(x_u .le. dNu+xnorm .and. x_u .ge. xnorm)then
                              ...
                              if(abs(uo(i,j,k)) .gt. max_velmag)then
                                 print*,'Parameterized U exceeds max in pentagon wake',&
                        ...
                        if(R_v .gt. radius_in .and.   &
                           ...
                           if(x_v .le. dNv+xnorm .and. x_v .ge. xnorm)then
                              ...
                              if(abs(vo(i,j,k)) .gt. max_velmag)then
                                 print*,'Parameterized V exceeds max in pentagon wake',&
...
! set up the wind parameterization in the courtyard MAN 4/25/05
                        ...
                        if(in_court .eq. 1.)then
                           in_courtwake_u=0
                           in_courtwake_v=0
                           x_nearu=max(real(nx)*dx,real(ny)*dy)
                           x_nearv=x_nearu
                           ...
                           if(in_courtwake_u .eq. 1)then
                              ...
                              if(i .lt. nx .and. j .lt. ny .and. k .lt. nz)then
                                 if(icellflag(i,j,k) .ne. 0)then
                                    if(zb .ge. 0.8*(Ht(ibuild)-zfo(ibuild)) .or. &
